...
- [ ] 4.1 本機啟動（`dotnet run` / IIS Express），觸發 EF AutoMigration Seed
- [ ] 4.2 SQL Server 檢查 `AdminUsers` 表有兩筆（`admin`、`superadmin`）
- [ ] 4.3 用 `superadmin` + 明文密碼登入 `/Admin/Account/Login`，確認進入 Branches 頁
- [ ] 4.4 用錯誤密碼登入，確認顯示「帳號或密碼錯誤」
- [ ] 4.5 透過 `/Admin/Account/ChangePassword` 改掉 `superadmin` 密碼，重新登入確認新密碼生效
